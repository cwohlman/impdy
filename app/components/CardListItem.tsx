...
export default function CardListItem({ card }: CardListItemParams) {
  return (
    <div className="my-2 flex overflow-hidden bg-white shadow sm:rounded-lg">
      <div className="grow">
        <div className={"p-2  " + (card.resolutionId ? "text-gray-700 text-sm" : "text-lg")}>{card.content}</div>
        <div className="p-2 text-sm text-gray-700 italic">
          {card.resolution ? (
            <div className="text-green-900 font-bold">{card.resolution.content}</div>
      ...
      <div className="flex flex-col">
        <Link to={"/card/" + card.id} prefetch="intent" className="inline-flex grow items-center justify-center rounded rounded-l-none rounded-b-none border border-transparent bg-blue-50 px-2.5 py-1.5 text-xs font-medium text-black shadow-sm hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
          Start
        </Link>
